...
test('returns expected output based on Sample Input 1', async () => {
    const { stdout } = await execa('./cli2.js', ['--input=samples/challenge2/input1.txt']);
...
//     const { stdout } = await execa('./cli2.js', ['--input=samples/challenge2/input2.txt']);
//     expect(stdout.trim()).toBe(fs.readFileSync('samples/challenge2/output2.txt', 'utf8').trim());
// });

// test('returns expected output based on Large Sample Input', async () => {
//     const { stdout, stderr } = await execa('./cli2.js', ['--input=samples/challenge2/input3.txt']);
//     expect(stdout.trim()).toBe(fs.readFileSync('samples/challenge2/output3.txt', 'utf8').trim());
// });

test('return error when input file is not specified', async () => {
    const { stdout, stderr } = await execa('./cli2.js').catch(err => err);
...
test(`return error when input file doesn't exist`, async () => {
    const { stdout, stderr } = await execa('./cli2.js', ['--input=foo.txt']).catch(err => err);
    expect(stdout || stderr).toContain('ENOENT: no such file or directory');
})

test('return error when input file is empty', async () => {
    const { stdout, stderr } = await execa('./cli2.js', ['--input=samples/challenge2/empty-input.txt']).catch(err => err);
    expect(stdout || stderr).toContain('Input file is empty');
});

test('return error when input file has invalid format 1', async () => {
    const { stdout, stderr } = await execa('./cli2.js', ['--input=samples/challenge2/invalid-input-1.txt']).catch(err => err);
    expect(stdout || stderr).toContain('Invalid input');
});

test('return error when input file has invalid format 2', async () => {
    const { stdout, stderr } = await execa('./cli2.js', ['--input=samples/challenge2/invalid-input-2.txt']).catch(err => err);
    expect(stdout || stderr).toContain('Invalid input');
});

test('return error when input file has invalid format 3', async () => {
    const { stdout, stderr } = await execa('./cli2.js', ['--input=samples/challenge2/invalid-input-3.txt']).catch(err => err);
    expect(stdout || stderr).toContain('Invalid input');
});

test('return error when input file has invalid format 4', async () => {
    const { stdout, stderr } = await execa('./cli2.js', ['--input=samples/challenge2/invalid-input-4.txt']).catch(err => err);
    expect(stdout || stderr).toContain('Invalid input');
});
